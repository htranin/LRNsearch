...
arglist = ["172.048831818","25.664042727", "AT2024mxe"]

if len(arglist)==3 and len(sys.argv)<4:
    sys.argv = [sys.argv[0]] + arglist
...
def query_sky_mapper_catalogue(ra, dec, radius_deg, minmag=15, maxmag=18.5):
    ...
    f = open("../Catalogs/skymapper_cat.csv", "wb")
    page = urlopen(url)
    content = page.read()
    f.write(content)
    f.close()
    ...
    catalog = Table.read("../Catalogs/skymapper_cat.csv", format="ascii.csv")
...
def query_ps1_catalogue(ra, dec, radius_deg, minmag=15, maxmag=18.5):
    ...
    #urllib.urlretrieve(url, "../Catalogs/ps1_cat.xml") 
    
    f = open("../Catalogs/ps1_cat.csv", "wb")
    ...
    # Read RA, Dec and magnitude from CSV 
    catalog = Table.read("../Catalogs/ps1_cat.csv", format="ascii.csv", header_start=1)
...
def get_fits_image(ra, dec, rad, debug=True):
    ...
    if dec > -30:
        ...
        urlretrieve(image_index_url, '../Catalogs/ps1_image_index.txt')
        ix = Table.read('../Catalogs/ps1_image_index.txt', format="ascii")
        ...
        urlretrieve(image_url, '../Catalogs/tmp.fits')
    ...
    #Otherwise, we have SkyMapper   
        ...
        page = urlopen(url)
        content = page.read()
        f = open("../Catalogs/skymapper_image_index.csv", "wb")
        f.write(content)
        f.close()
        
        ix = Table.read('../Catalogs/skymapper_image_index.csv', format="ascii.csv")
...
def get_cutout(ra, dec, name, rad, debug=True):
    ...
    urlretrieve(image_index_url_red, '../Catalogs/image_index_red.txt')
    urlretrieve(image_index_url_green, '../Catalogs/image_index_green.txt')
    urlretrieve(image_index_url_blue, '../Catalogs/image_index_blue.txt')

    ix_red = np.genfromtxt('../Catalogs/image_index_red.txt', names=True, dtype=None)
    ix_green = np.genfromtxt('../Catalogs/image_index_green.txt', names=True, dtype=None)
    ix_blue = np.genfromtxt('../Catalogs/image_index_blue.txt', names=True, dtype=None)
    ...
    if (debug):
        print (image_url)
        print ("Downloading PS1 r-band image...")
    urlretrieve(image_url, '../Catalogs/tmp_%s.jpg'%name)
